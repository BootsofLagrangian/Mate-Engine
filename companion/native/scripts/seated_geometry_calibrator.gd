class_name SeatedGeometryCalibrator
extends RefCounted
## One-time canonical-pose measurement. No live bounds chasing or rig mutation.
static func measure(avatar: VrmAvatar, rotations: Dictionary, preserve_pose: bool = false, allow_floor: bool = true) -> Dictionary:
	...
	if not patch.is_empty():
		...
		anchor.y=minimum
	var constrained := {}
	if allow_floor and is_finite(avatar.seated_floor.clearance):
		avatar.seated_floor.floor_y=anchor.y-avatar.seated_floor.clearance
		var springs_before:=avatar.seated_floor.snapshot_springs()
		avatar.seated_floor.solve_legs()
		avatar.seated_floor.solve_secondary_pose()
		constrained=measure(avatar,rotations,true,false)
		avatar.seated_floor.restore_springs(springs_before)
		if not constrained.is_empty():
			constrained.diagnostics["floor_y"] = avatar.seated_floor.floor_y
			constrained.diagnostics["floor_mesh_clearance"] = constrained.bounds.position.y-avatar.seated_floor.floor_y
	...
	if not constrained.is_empty(): return constrained
	if first: return {}
	return {"anchor":to_avatar*anchor,"bounds":bounds,"diagnostics":{"vertices":count,"secondary_samples":avatar._secondary_pose_samples,"secondary_cached":not avatar._secondary_pose_cache.is_empty(),"patch_vertices":patch.size(),"contact_band_vertices":band,"bind_rest_max_error_m":rest_error,"mesh_min_y":mesh_extents,"method":"skinned_pelvis_thigh" if band>0 else "hip_fallback"}}
