...
func mesh_min(p:MotionPlayer)->float:
 ...
 return lowest
func run()->void:
 for character in (["mambo"] if OS.get_environment("CONTACT_MINI")=="1" else ["cheval-grand","rice-shower","eishin-flash"]):
  for fps in [30,60]:
   for sizing in ["source","fixed"]:
    if character=="mambo" and sizing=="fixed":continue
 ...
 var out:=FileAccess.open("/tmp/authored-contact-path-mini.json" if OS.get_environment("CONTACT_MINI")=="1" else "/tmp/authored-contact-path.json",FileAccess.WRITE);out.store_string(JSON.stringify({"failures":failures,"cases":reports},"  "))
