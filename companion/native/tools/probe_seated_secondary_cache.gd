...
func run() -> void:
	...
	for character in ["cheval-grand","rice-shower","eishin-flash"]:
		...
		print("CACHE_MATCH ",character," cached_bottom=",cached_bottom," observed=",observed," undershoot_m=",worst)
		if avatar._secondary_pose_samples<30 or observed.size()!=4 or worst>0.002: failures+=1
		root.get_texture().get_image().save_png(output.path_join(character+"-cached-seat.png"))
		player.free()
		avatar.free()
	print("RENDERS=",output)
	print("SEATED_CACHE_FAILURES=",failures)
