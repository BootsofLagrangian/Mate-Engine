...
var output:="/tmp/cape-continuous-seated"
...
func hashes()->Dictionary:
 var result:={}
 for path in ["res://tools/probe_cape_continuous.gd","res://tools/garment_quality_samples.gd","res://scripts/motion_player.gd","res://scripts/seated_transition.gd","res://scripts/vrm_avatar.gd","res://scripts/desktop_gait.gd","res://../assets/cheval-grand.vrm","res://../assets/research/cape-candidates/cyspring-cloth00.json"]:
...
func run()->void:
 ...
 for f in 1320:
  ...
  var heading:=deg_to_rad(110.0)*smoothstep(240.0,360.0,float(f))
 ...
 var report:={"source_start":source_start,"source_end":hashes(),"parameter_endpoints":parameter_endpoints,"max_world_transform_error":max_world_error,"scope":"actual MotionPlayer shared scripted events; automatic idle selection disabled, authored idle retained; own-world twin viewports same-origin/identical transforms; real modifier callbacks; cape-only sparse triangles every17th; model reload same asset with translated/scaled placement; actual seated transition layer and .45m floorclearance, source rootdelta applied by diagnostic host, no chair/backrest and no host/window acceptance","frames":1320,"fps":60,"capture_fps":30,"counts":counts,"max_prephysics_humanoid_pose_error":max_source_error,"parameters":parameter_checks,"callbacks":rows,"mesh_samples":meshes}
