...
func run() -> void:
	...
	for character in [OS.get_environment("WALK_CHARACTER") if not OS.get_environment("WALK_CHARACTER").is_empty() else "cheval-grand"]:
		...
		stage.add_child(player);player.set_process(false);player.avatar=avatar;player.set_bank(bank)
		player.idle_enabled=false;player.gaze_enabled=false
		for name in ["uma_walk","authored_wave","authored_bow"]:
			player.load_vrma(name,ProjectSettings.globalize_path("res://../assets/motions/"+name+".vrma"))
		player.register_locomotion_clip("uma_walk",true)
		for frame in 30:player._process(1.0/30)
		if selected=="walk_upper":
			player.prepare_locomotion(Vector2(100,0));player.play_vrma("uma_walk",1,true)
		else:player.play_vrma("authored_wave")
		for frame in 150:
			if selected=="walk_upper":
				if frame==30:player.play_upper_body_gesture("wave")
				if frame==60:player.finish_locomotion();player.play_gesture("bow")
			elif frame==24:player.play_vrma("authored_bow")
			player._process(1.0/30)
			if selected=="walk_upper" and frame<60:
				var dx:=80.0/30
				var ppm:=600.0/1.95
				avatar.position.x+=dx/ppm;camera.position.x=avatar.position.x
				player.set_locomotion_sample(Vector2(80,0),Vector2(dx,0),ppm,true)
			await process_frame
			await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png(output.path_join("seam-%03d.png" % frame))
		player.free()
