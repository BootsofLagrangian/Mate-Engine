...
func run() -> void:
	...
	for character in ["cheval-grand","rice-shower","eishin-flash"]:
		...
		player.load_vrma("dance", ProjectSettings.globalize_path("res://../assets/motions/quaternius-dance.vrma"))
		for gesture in ["idle","wave","bow","stretch","dance"]:
			if gesture == "dance":
				player.play_vrma("dance")
			else:
				player.play_gesture(gesture)
			for i in (30 if gesture == "dance" else 90):
