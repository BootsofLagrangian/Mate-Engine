...
소품은 **LM이 필요할 때 호출하는 능력**으로 연결됩니다. “컴퓨터를 꺼내서 써 봐”처럼
말하면 등록된 소품 규약에 따라 생성·배치·접근·사용을 실행하고 결과를 돌려줍니다.
의자·소파·컴퓨터의 회전, 크기, 재질 프리셋도 같은 경로로 제어합니다.
[공간·능력 규약](diagnostics/desktop_objects/FURNITURE-INTENT-CONTRACT.md)을 참고하세요.
**공간** 탭은 선택적인 편집·확인 도구입니다. 실제 프로그램에 키를 입력하는 기능은
아직 포함하지 않으며, 들고 다니는 소품·도구는 후속 확장입니다.

기본 실행은 **투명·무장식 펫 모드**이며 대기·간헐적 산책이 켜져 있습니다. Windows는
Direct3D 12 / Forward+를 사용해 실제 바탕화면 합성의 검은 배경 문제를 해결합니다.
F8 설정은 캐릭터와 별도 창으로 열립니다. **시점** 탭에서 좌우 회전, 위아래 각도,
눈높이와 확대를 조절할 수 있으며 소품 자체의 회전과 구분됩니다.
기본 보행은 설치된 `uma_walk`이며 상체 손짓을 하체 보행과 별도로 재생할 수 있습니다.
