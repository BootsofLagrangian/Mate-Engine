...
class HeadingMotion:
	extends Motion
	var first_step_ready := false
	func prepare_locomotion(direction: Vector2) -> bool:
		set_heading_intent(direction)
		return true
...
func _run() -> void:
	...
	host._wire()
	host.autonomy.set_heading_ready_provider(func(): return host.motion.locomotion_ready(), 0.25)
	...
	check(host.autonomy.state == "anticipate" and host.autonomy.position == origin,"main readiness closure blocks translation before the first aligned step")
	host.motion.first_step_ready = true
	host.autonomy.advance(0.1)
	check(host.autonomy.state == "walk", "first-step readiness releases movement without waiting for full heading")
