...
func run() -> void:
		...
		for yaw in [0.0,45.0,90.0]:
			for pitch in [45.0]:
				for scale in [0.6,1.0]:
					settings.data.merge({"view_projection":"perspective","view_yaw_deg":yaw,"view_pitch_deg":pitch,"view_height":0.0,"view_zoom":1.0},true)
					root.position=Vector2i(1000,500)
					app._standing_floor_y=NAN;app._standing_floor_id=""
					app._spatial_origin=Vector2(root.position)
					...
					a.configure_simulation([area],Vector2(root.position),app._navigation_rect())
					...
					a.set_world_snapshot({"monitors":[{"id":"test","work_x":0,"work_y":0,"work_width":2560,"work_height":area.end.y}],"windows":[]})
					a._support=a._surfaces.get_surfaces(1)[0].duplicate()
					a._locked_anchor=anchor;a._anchors={"foot":anchor};a.target=a.position
					root.position=Vector2i(a.position.round())
					app._update_avatar_transform(0)
					app._on_support_changed(a.get_support_contact())
					...
					for frame in 121:
						...
						max_plane_error=maxf(max_plane_error,absf((app.avatar.global_transform*Vector3(app._pivot_local.foot)).y-app._standing_floor_y))
	...
	FileAccess.open("res://../diagnostics/desktop_view/floor-support-perspective-commit-results.json",FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
