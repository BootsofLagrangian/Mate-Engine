...
func run():
 ...
 for fps in [60,90,144]:
  for character in ["cheval-grand","rice-shower","eishin-flash"]:
   ...
   rows.append({"character":character,"fps":fps,"duration_s":p.seated_transition.duration,"whole_clip_grid_interval_s":p.seated_transition.duration/12,"max_acquisition_bounds_overflow_m":maximum})
   p.free();a.free()
 FileAccess.open(ProjectSettings.globalize_path("res://../diagnostics/motion_seams/review-bounds.json"),FileAccess.WRITE).store_string(JSON.stringify(rows,"  ")+"\n")
