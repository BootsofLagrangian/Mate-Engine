...
func run()->void:
 ...
 for character in ["cheval-grand","rice-shower","eishin-flash","mambo","hachimi"]:
  ...
  if not p.seated_carrier_lift_envelope().is_empty():failures+=1
  p.set_seated_carrier(0,0)
  if not p.seated_carrier_lift_envelope().is_empty():failures+=1
  ...
  for frame in 483:
   ...
   if frame in [0,241,482]:
    var duplicate:=p.seated_carrier_lift_envelope()
    if duplicate!=envelope:failures+=1
    duplicate.capsules[0].radius=999
    if p.seated_carrier_lift_envelope().capsules[0].radius==999:failures+=1
  ...
  if overflow>.000001 or mutation>0:failures+=1
  rows.append({"character":character,"overflow_m":overflow,"pose_mutation":mutation,"maximum_margin_m":margin,"max_query_us":max_query_us,"samples":483,"capsules":envelope.get("capsules",[]).size()})
  if not p.set_seated_floor(clearance*1.01):failures+=1
  if not p.seated_carrier_lift_envelope().is_empty() or not p.seated_carrier_body_snapshot().is_empty():failures+=1
  p._process(1.0/60)
  if p.seated_carrier.active:failures+=1
  p.free();a.free()
 FileAccess.open("/tmp/lift-envelope-review.json",FileAccess.WRITE).store_string(JSON.stringify({"failures":failures,"rows":rows},"  "))
