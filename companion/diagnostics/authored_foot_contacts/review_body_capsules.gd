...
func mesh_min(p:MotionPlayer)->float:
 ...
 return lowest
func run()->void:
 for scale in [.001,1.0,3.0]:
  var basis:=Basis(Vector3(scale,0,0),Vector3(scale*.5,scale,0),Vector3(0,0,scale))
  var bound:=RigBodyCapsules.scale_bound(basis)
  for i in 360:
   var direction:=Vector3(cos(i*TAU/360),sin(i*TAU/360),0)
   if (basis*direction).length()>bound+1e-8:failures+=1
 for character in ["cheval-grand","rice-shower","eishin-flash","mambo","hachimi"]:
  ...
  for frame in 156:
   ...
   previous=points
   if frame<30 and not p.seated_carrier_body_snapshot().is_empty():failures+=1
  ...
  p.set_seated_carrier(1,a.rotation.y)
  var held_avatar:=p.avatar
  p.avatar=null
  if not p.seated_carrier_body_snapshot().is_empty():failures+=1
  p.avatar=held_avatar
  var restart:=p.start_contact_pose("sit")
  p._process(1.0/60)
  # A restart must either reject without changing ownership, or retire the old capture.
  if restart and p.seated_carrier.active:failures+=1
  p.play_vrma("sit_idle",1,true)
  var new_playback_start:=p._vrma_start
  p._process(1.0/60)
  if p.seated_carrier.active or p._vrma_start!=new_playback_start:failures+=1
  p.set_seated_carrier(1,a.rotation.y)
  ...
  p.free();a.free()
 FileAccess.open("/tmp/body-capsules-review.json",FileAccess.WRITE).store_string(JSON.stringify({"failures":failures,"rows":rows},"  "))
