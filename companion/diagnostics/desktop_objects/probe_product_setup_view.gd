...
func run()->void:
 ...
 var actor:Vector3=DesktopView.solve_screen_at_world_y(camera,Vector2(2382,1392)-DESKTOP_ORIGIN,target.y).point
 ...
 var plan:=DesktopWorkstationSetup.plan(scene,actor,.6,offset,source,.072,avatar.compute_aabb().size.y*.6,[],fitting)
 var summary:=plan.duplicate();summary.erase("geometric_candidate");print("PRODUCT_VIEW_ALIGNED_PLAN ",summary)
 ...
 if okay:
  ...
  var proposal:=DesktopWorkstationSetup.reposition(scene,plan.geometric_candidate,camera,DESKTOP_ORIGIN,[Rect2(0,0,2560,1392)],actor,.072,avatar.compute_aabb().size.y*.6,[],.9)
  print("PRODUCT_VIEW_REPOSITION ",proposal," actor=",actor," planning_ms=",(Time.get_ticks_usec()-reposition_started)/1000.0)
  ...
  if okay:
   ...
   var final_plan:=DesktopWorkstationSetup.plan(scene,actor,.6,offset,source,.072,avatar.compute_aabb().size.y*.6,[],fitting)
   summary=final_plan.duplicate();summary.erase("setup_parts_local");print("PRODUCT_VIEW_FRESH_PLAN ",summary)
   ...
   if okay:
    ...
    okay=continuous_fits;print("PRODUCT_VIEW_RUNTIME_62_POSES_FIT=",continuous_fits)
 print("PRODUCT_VIEW_FIXED_PASS=",okay)
